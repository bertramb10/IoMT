import network
import time
import ntptime  # NTP for time sync
...
# I2C Bus (Shared for Both Sensors)
...
def reset_fifo():
    write_register(REG_FIFO_WR_PTR, 0x00)  # Reset write pointer
...
def vibrate_at_13():
    while True:
        ...
        if current_time[3] == 13 and current_time[4] == 0:  # Timer er 13:00
            ...
            vibration_motor.value(1)  # Tænd motoren
            time.sleep(30)  # Vent i 30 sekunder
            vibration_motor.value(0)  # Sluk motoren
        time.sleep(60)  # Tjek hver minut
...
def main():
    ...
    # Start tråden til overvågning af klokken (Vibrationsmotor ved 13:00)
    vibration_thread = threading.Thread(target=vibrate_at_13)
    vibration_thread.daemon = True  # Gør tråden til en daemon-tråd
    vibration_thread.start()

    while True:
        try:
            # Restart program on button press
            ...
            if fifo_data:
                ...
                if red > 0 and ir > 0:  # Ensure valid data
            ...
            time.sleep(2)  # 2-second interval between reads

        except Exception as e:
            print(f"Error: {e}")
            client.connect()  # Reconnect MQTT if disconnected
